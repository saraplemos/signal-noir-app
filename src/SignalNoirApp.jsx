...
import DecisionInfluenceMatrix from './DecisionInfluenceMatrix';
...
const getMatrixPosition = (scores) => {
  const upstream = (scores.authority * 0.4 + scores.topical * 0.4 + scores.social * 0.2) / 10;
  const downstream = (scores.content * 0.5 + scores.search * 0.5) / 10;
  const total = upstream + downstream;
  const x = total > 0 ? (downstream / total) * 100 : 50;
  const y = ((scores.authority + scores.topical) / 2) / 10 * 10;
  return { x: Math.min(92, Math.max(8, x)), y: Math.min(92, Math.max(8, y)) };
};

const getQuadrantLabel = (x, y) => {
  const right = x >= 50, top = y >= 50;
  if (top && right)  return 'Dominant influence';
  if (top && !right) return 'Editorial authority, lost value';
  if (!top && right) return 'Conversion-led';
  return 'Invisible';
};
...
const SignalNoirApp = () => {
  ...
  const [selectedMatrixPub, setSelectedMatrixPub] = useState(null);
    ...
    try {
      ...
      if (Array.isArray(data.aiCitationsData)) {
        ...
        aiRows.forEach(row => {
          ...
          if (!name) return;
          breakdown[name] = {
            chatgpt:    parseFloat(row[1]) || 0,
            claude:     parseFloat(row[2]) || 0,
            perplexity: parseFloat(row[3]) || 0,
            gemini:     parseFloat(row[4]) || 0,
  ...
  const fetchFromCSV = async () => {
    try {
      ...
      const pubs = dataRows
        .map((row, index) => ({
          id: index + 1, name: row[0] || '', row: index + DATA_FIRST_ROW,
          scores: {
            authority:   parseFloat(row[3]) || 0,
            aiCitations: parseFloat(row[4]) || 0,
            content:     parseFloat(row[5]) || 0,
  ...
  const updateScore = (pubId, signal, value) => {
    ...
    setPublications(publications.map(pub =>
      pub.id === pubId ? { ...pub, scores: { ...pub.scores, [signal]: numValue } } : pub
    ));
  };
  ...
  const QUERY_DATA = [
    { id:1, topic:'Luxury hotel Dubai', category:'Hotels',
      chatgpt:[{n:'The Times Travel',t:'editorial'},{n:'Condé Nast Traveller UK',t:'editorial'},{n:'Condé Nast Traveler US',t:'editorial'},{n:'Four Seasons Press Room',t:'official'}],
      perplexity:[{n:'LOCALS Insider',t:'blog'}],
      gemini:[{n:'Condé Nast Traveler',t:'editorial'},{n:'Forbes Travel Guide',t:'editorial'},{n:'Four Seasons Press Room',t:'official'}],
      claude:[{n:'MICHELIN Guide',t:'editorial'},{n:'TripAdvisor',t:'aggregator'},{n:'Expedia',t:'aggregator'},{n:'MakeMyTrip',t:'aggregator'}]},
    ...
    { id:3, topic:'Ski chalet Courchevel', category:'Ski',
      ...
      perplexity:[{n:'Ski In Luxury',t:'specialist'},{n:'Alpine Luxury Chalets',t:'specialist'}],
      gemini:[{n:'Kaluma Ski',t:'specialist'},{n:'Telegraph Travel',t:'editorial'},{n:'Forbes Travel Guide',t:'editorial'}],
      claude:[{n:'Courchevel.com',t:'blog'},{n:'Ski In Luxury',t:'specialist'},{n:'Le Collectionist',t:'specialist'},{n:'Ultimate Luxury Chalets',t:'specialist'}]},
    { id:4, topic:'Luxury ski resort Verbier', category:'Ski',
      chatgpt:[{n:'TripAdvisor',t:'aggregator'},{n:'Experimental Chalet',t:'official'}],
      perplexity:[{n:'Oxford Ski',t:'specialist'},{n:'Ski In Luxury',t:'specialist'}],
    ...
    { id:5, topic:'Luxury resort Maldives', category:'Islands',
      ...
      perplexity:[{n:'YouTube (Tourpoint)',t:'youtube'}],
      gemini:[{n:'YouTube (Top 10 Luxury Resorts)',t:'youtube'},{n:'Condé Nast Traveler',t:'editorial'},{n:'Travel + Leisure',t:'editorial'}],
      claude:[{n:'Dorsia Travel',t:'blog'},{n:'Maldives Magazine',t:'blog'},{n:'The Asia Collective',t:'specialist'},{n:'Jetset & Travel',t:'blog'}]},
    { id:6, topic:'Private island Maldives', category:'Islands',
      chatgpt:[{n:'Forbes Travel Guide (Velaa)',t:'editorial'},{n:'Forbes Travel Guide (Kudadoo)',t:'editorial'}],
    ...
    { id:7, topic:'Private island Caribbean', category:'Islands',
      chatgpt:[{n:'Condé Nast Traveler',t:'editorial'},{n:'Condé Nast Traveller UK',t:'editorial'}],
      perplexity:[{n:'Ambergris Cay',t:'official'},{n:'Carib Journal',t:'blog'}],
      gemini:[{n:'Lifestyle Travel Network',t:'blog'},{n:'Travel + Leisure',t:'editorial'},{n:'Virgin Limited Edition',t:'official'}],
      claude:[{n:'Private Islands Online',t:'aggregator'},{n:'Vladi Private Islands',t:'specialist'},{n:'WIMCO Villas',t:'specialist'},{n:'Mustique Island',t:'official'}]},
    { id:8, topic:'Luxury hotel Lake Como', category:'Hotels',
      ...
      claude:[{n:'Elite Traveler',t:'editorial'},{n:'Hotels.com',t:'aggregator'},{n:'Travelocity',t:'aggregator'},{n:'Luxury Escapes',t:'aggregator'}]},
    { id:9, topic:'Luxury hotel Amalfi Coast', category:'Hotels',
      ...
      perplexity:[{n:'Luxury.it',t:'blog'}],
      gemini:[{n:"Lulu's Luxury Lifestyle",t:'blog'},{n:'Forbes Travel Guide',t:'editorial'},{n:'Belmond',t:'official'}],
      claude:[{n:'Mr & Mrs Smith',t:'specialist'},{n:'TripAdvisor',t:'aggregator'},{n:'Luxury Escapes',t:'aggregator'},{n:'American Express Travel',t:'aggregator'}]},
    { id:10, topic:'Boutique hotel French Riviera', category:'Hotels',
      ...
      perplexity:[{n:'Les Boutique Hotels',t:'blog'}],
      gemini:[{n:'Tablet Hotels',t:'specialist'},{n:'Michelin Guide',t:'editorial'},{n:'Mr & Mrs Smith',t:'specialist'}],
      claude:[{n:'Mr & Mrs Smith',t:'specialist'},{n:'Small Luxury Hotels',t:'specialist'},{n:'TripAdvisor',t:'aggregator'},{n:'Booking.com',t:'aggregator'}]},
    { id:11, topic:'Luxury hotel Kyoto', category:'Hotels',
      ...
      claude:[{n:'MICHELIN Guide',t:'editorial'},{n:'Mr & Mrs Smith',t:'specialist'},{n:'TripAdvisor',t:'aggregator'},{n:'Hotel The Mitsui Kyoto',t:'official'}]},
    { id:12, topic:'Luxury safari lodge Tanzania', category:'Safari',
      ...
      perplexity:[{n:'Tanzania National Parks',t:'official'},{n:'Go2Africa',t:'specialist'}],
      gemini:[{n:'Africa Travel',t:'specialist'},{n:'Ubuntu Travel Group',t:'specialist'},{n:'Singita',t:'official'}],
      claude:[{n:'TripAdvisor',t:'aggregator'},{n:'Serengeti.com',t:'blog'},{n:'Go2Africa',t:'specialist'},{n:'Singita',t:'official'}]},
    { id:13, topic:'Private museum tour Paris', category:'Experiences',
      ...
      perplexity:[{n:'Paris Luxury Tours',t:'specialist'},{n:'Paris By Emy',t:'specialist'},{n:'My Private Paris',t:'specialist'}],
      gemini:[{n:'Grand Hôtel du Palais Royal',t:'official'},{n:'My Private Paris',t:'specialist'},{n:'Paris to Versailles Tours',t:'specialist'}],
      claude:[{n:'Louvre (official)',t:'official'},{n:'Paris Muse',t:'specialist'},{n:'Viator',t:'aggregator'},{n:'GetYourGuide',t:'aggregator'}]},
    { id:14, topic:'Private Vatican tour', category:'Experiences',
      ...
      perplexity:[{n:'Vatican Tickets Online',t:'specialist'},{n:'LivTours',t:'specialist'},{n:'Italy by Luxe',t:'specialist'}],
      gemini:[{n:'The Tour Guy',t:'specialist'},{n:'The Vatican Tickets',t:'specialist'}],
      claude:[{n:'Vatican Museums (official)',t:'official'},{n:'Walks of Italy',t:'specialist'},{n:'LivTours',t:'specialist'},{n:'The Roman Guy',t:'specialist'}]},
    ...
    { id:16, topic:'Wellness retreat', category:'Wellness',
      chatgpt:[{n:'Condé Nast Traveller UK',t:'editorial'},{n:'Condé Nast Traveller ME',t:'editorial'},{n:'Condé Nast Traveler US',t:'editorial'}],
      ...
      claude:[{n:'SHA Wellness Clinic',t:'official'},{n:'COMO Hotels',t:'official'},{n:'Condé Nast Traveler',t:'editorial'},{n:'SpaFinder',t:'aggregator'}]},
    { id:17, topic:'Medical wellness program', category:'Wellness',
      chatgpt:[{n:'Clinique La Prairie',t:'official'},{n:'Lanserhof',t:'official'},{n:'SHA Wellness Clinic',t:'official'}],
      perplexity:[{n:'GetTransfer Blog',t:'blog'},{n:'RAKxa Integrative Wellness',t:'official'}],
      gemini:[{n:'Lanserhof',t:'official'},{n:'Wellbeing Escapes',t:'specialist'},{n:'SHA Wellness Clinic',t:'official'}],
      claude:[{n:'Lanserhof',t:'official'},{n:'Chenot',t:'official'},{n:'Six Senses',t:'official'},{n:'Medical Tourism Association',t:'specialist'}]},
    { id:18, topic:'Antarctic expedition cruise', category:'Adventure',
      chatgpt:[{n:'Quark Expeditions',t:'specialist'},{n:'Silversea',t:'official'},{n:'PONANT',t:'official'},{n:'National Geographic Expeditions',t:'editorial'}],
      perplexity:[{n:'Antarctica Cruises',t:'specialist'},{n:'Swoop Antarctica',t:'specialist'},{n:'Cruise Critic',t:'aggregator'}],
      gemini:[{n:'Silversea',t:'official'},{n:'Luxury Check-In',t:'blog'},{n:'Ponant',t:'official'}],
      claude:[{n:'Quark Expeditions',t:'specialist'},{n:'Silversea',t:'official'},{n:'IAATO',t:'official'},{n:'Cruise Critic',t:'aggregator'}]},
    { id:19, topic:'Northern lights private charter', category:'Adventure',
      ...
      claude:[{n:'Guide to Iceland',t:'blog'},{n:'Hurtigruten',t:'official'},{n:'Visit Norway',t:'official'},{n:'Aurora Forecast',t:'specialist'}]},
    { id:20, topic:'Heli-skiing British Columbia', category:'Adventure',
      ...
      perplexity:[{n:'Bella Coola Heli Sports',t:'official'},{n:'Mabey Ski',t:'specialist'}],
      gemini:[{n:'CMH Heli-Skiing',t:'official'},{n:'Bella Coola Heli Sports',t:'official'},{n:'Global Air Charters',t:'specialist'}],
      claude:[{n:'CMH Heli-Skiing',t:'official'},{n:'Bella Coola Heli Sports',t:'official'},{n:'HeliCat Canada',t:'official'},{n:'Powder Magazine',t:'editorial'}]},
    { id:21, topic:'Venice Simplon Orient Express', category:'Transport',
      chatgpt:[{n:'Belmond',t:'official'},{n:'Condé Nast Traveller UK',t:'editorial'},{n:'Condé Nast Traveler US',t:'editorial'}],
      perplexity:[{n:'Luxury Train Tickets',t:'specialist'},{n:'Railtour',t:'specialist'}],
      gemini:[{n:'Belmond',t:'official'},{n:'Condé Nast Traveler',t:'editorial'}],
      claude:[{n:'Belmond',t:'official'},{n:'Condé Nast Traveler',t:'editorial'},{n:'Travel + Leisure',t:'editorial'}]},
    { id:22, topic:'Private safari lodge Botswana', category:'Safari',
      chatgpt:[{n:'Expert Africa',t:'specialist'},{n:'Wilderness (Mombo)',t:'official'},{n:'Great Plains Conservation',t:'official'}],
      perplexity:[{n:'The Luxury Travel Expert',t:'blog'},{n:'Passport and Pixels',t:'blog'}],
    ...
    { id:23, topic:'Villa with chef & staff Tuscany', category:'Villas',
      ...
      perplexity:[{n:'Coselli',t:'specialist'},{n:'Arianna and Friends',t:'specialist'},{n:'Il Casale del Marchese',t:'official'}],
      gemini:[{n:'Tuscan Dream',t:'specialist'},{n:'Kinglike Concierge',t:'specialist'}],
    ...
    { id:24, topic:'Ski chalet with catering Verbier', category:'Ski',
      chatgpt:[{n:'Verbier Exclusive',t:'specialist'},{n:'Ultimate Luxury Chalets',t:'specialist'},{n:'Leo Trippi',t:'specialist'},{n:'SeeVerbier',t:'blog'}],
      perplexity:[{n:'David Pearson Travel',t:'specialist'},{n:'Luxury Chalet Co.',t:'specialist'},{n:'Verbier.co',t:'blog'}],
      gemini:[{n:'Verbier Exclusive',t:'specialist'},{n:'Oxford Ski Company',t:'specialist'}],
    ...
    { id:25, topic:'Yacht charter Mediterranean', category:'Yachts',
      ...
      gemini:[{n:'Fraser Yachts',t:'specialist'},{n:'Sunsail',t:'specialist'}],
      claude:[{n:'Fraser Yachts',t:'specialist'},{n:'Burgess',t:'specialist'},{n:'Boat International',t:'editorial'},{n:'CharterWorld',t:'aggregator'}]},
    { id:26, topic:'Private island resort with butler Maldives', category:'Islands',
      chatgpt:[{n:'St. Regis Maldives',t:'official'},{n:'St. Regis Butler Service',t:'official'},{n:'SUNxSIYAM Resorts',t:'official'}],
      perplexity:[{n:'Go Ocean Travel',t:'blog'}],
      gemini:[{n:'The Ritz-Carlton Maldives',t:'official'},{n:'St. Regis Maldives',t:'official'}],
      claude:[{n:'Condé Nast Traveller',t:'editorial'},{n:'Mr & Mrs Smith',t:'specialist'},{n:'Velaa Private Island',t:'official'},{n:'Cheval Blanc',t:'official'}]},
    { id:27, topic:'Multi-generational villa rental', category:'Villas',
      chatgpt:[{n:'onefinestay',t:'specialist'},{n:'Mandarin Oriental Exceptional Homes',t:'official'},{n:'Le Collectionist',t:'specialist'}],
      perplexity:[{n:'Villas of Distinction',t:'specialist'},{n:'CV Villas',t:'specialist'}],
      gemini:[{n:'Tripwix',t:'specialist'},{n:'Bailey Robinson',t:'specialist'}],
      claude:[{n:'Scott Dunn',t:'specialist'},{n:'CV Villas',t:'specialist'},{n:'VillaNoVo',t:'specialist'},{n:'Travel + Leisure',t:'editorial'}]},
    { id:28, topic:'Family estate rental', category:'Villas',
      chatgpt:[{n:"Oliver's Travels",t:'specialist'},{n:'Le Collectionist',t:'specialist'},{n:'Mandarin Oriental Exceptional Homes',t:'official'}],
      ...
      claude:[{n:'Unique Homestays',t:'specialist'},{n:'Rural Retreats',t:'specialist'},{n:'Condé Nast Traveller',t:'editorial'},{n:'Historic Houses',t:'specialist'}]},
    { id:29, topic:'Bespoke tour operators Antarctica', category:'Adventure',
      chatgpt:[{n:'Black Tomato',t:'specialist'},{n:'Abercrombie & Kent',t:'specialist'},{n:'Quark Expeditions',t:'specialist'}],
      perplexity:[{n:'Extraordinary Journeys',t:'specialist'}],
      gemini:[{n:'Cruise Critic',t:'aggregator'},{n:'Scenic Luxury Cruises',t:'official'}],
      claude:[{n:'White Desert',t:'specialist'},{n:'Silversea',t:'official'},{n:'Abercrombie & Kent',t:'specialist'},{n:'Cooksons Adventures',t:'specialist'}]},
    { id:30, topic:'Private jet charter London-New York', category:'Transport',
      ...
      perplexity:[{n:'Private Jet London to New York',t:'specialist'}],
      gemini:[{n:'ACC Aviation',t:'specialist'},{n:'Global Charter',t:'specialist'}],
      claude:[{n:'VistaJet',t:'official'},{n:'NetJets',t:'official'},{n:'PrivateFly',t:'specialist'},{n:'FlyVictor',t:'specialist'}]},
  ];

  // ── Matrix Panel ──────────────────────────────────────────────────────────
  const MatrixPanel = () => {
    const activePub = selectedMatrixPub
      ? sortedPublications.find(p => p.id === selectedMatrixPub)
      : sortedPublications[0];

    const QUADRANT_CONFIG = [
      { key: 'dominant',   label: 'Dominant influence',            color: '#FFD700', test: (x,y) => x >= 50 && y >= 50 },
      { key: 'editorial',  label: 'Editorial authority, lost value', color: '#60a5fa', test: (x,y) => x < 50  && y >= 50 },
      { key: 'conversion', label: 'Conversion-led',                 color: '#f97316', test: (x,y) => x >= 50 && y < 50  },
      { key: 'invisible',  label: 'Invisible',                       color: '#6b7280', test: (x,y) => x < 50  && y < 50  },
    ];
    ...
    return (
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          <h2 className="text-lg font-light tracking-wide">Decision Influence Matrix</h2>
          <select
            value={selectedMatrixPub || sortedPublications[0]?.id || ''}
            onChange={e => setSelectedMatrixPub(parseInt(e.target.value))}
            className="bg-gray-800 border border-gray-700 text-gray-300 text-sm rounded-lg px-4 py-2 focus:outline-none"
        ...
        {activePub && (
          <div className="bg-gray-900/50 rounded-lg border border-gray-800 p-6 mb-6">
            <DecisionInfluenceMatrix
              brandName={activePub.name}
              scores={{
                d1: activePub.scores.authority / 10,
                d2: activePub.scores.aiCitations / 10,
                d3: activePub.scores.content / 10,
                d4: activePub.scores.topical / 10,
                d5: activePub.scores.search / 10,
                d6: activePub.scores.social / 10
              }}
            />
          </div>
        )}

        {/* All publications quadrant summary */}
        <div className="bg-gray-900/50 rounded-lg border border-gray-800 p-5">
          <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-4">
            All publications — quadrant positions
          </h3>
          <div className="grid grid-cols-2 gap-4">
            {QUADRANT_CONFIG.map(q => {
              const pubsInQ = sortedPublications.filter(p => {
                const pos = getMatrixPosition(p.scores);
                return q.test(pos.x, pos.y);
              });
              return (
                <div key={q.key} className="rounded-lg p-4"
                  style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                  <div className="text-xs font-medium mb-1" style={{ color: q.color }}>{q.label}</div>
                  <div className="text-3xl font-mono font-bold mb-3" style={{ color: q.color }}>
                    {pubsInQ.length}
                  </div>
                  <div className="space-y-1">
                    {pubsInQ.slice(0, 6).map(p => (
                      <button
                        key={p.id}
                        onClick={() => setSelectedMatrixPub(p.id)}
                        className="block text-xs text-gray-500 hover:text-gray-300 text-left truncate w-full transition-colors">
                        {p.name}
                      </button>
                    ))}
                    {pubsInQ.length > 6 && (
                      <div className="text-xs text-gray-700">+{pubsInQ.length - 6} more</div>
                    )}
                  </div>
                </div>
              );
  ...
  const QueryResultsPanel = () => {
    ...
    const filtered = QUERY_DATA.filter(q => catFilter === 'All' || q.category === catFilter);
      ...
      <div className="mb-6">
        ...
        <div className="rounded-lg p-4 mb-5" style={{ background: 'rgba(255,215,0,0.05)', border: '1px solid rgba(255,215,0,0.2)' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold mb-1" style={{ color: '#FFD700' }}>Why 4/30 queries showed platform consensus</div>
                  ...
                  {['chatgpt','perplexity','gemini','claude'].map(p => {
                    ...
                    return (
                      <div key={p}>
                        ...
                        <div className="flex flex-wrap">{renderSources(q[p], p)}</div>
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="mb-6">
        ...
        <div className="mb-6 bg-gray-900/50 rounded-lg border border-gray-800 overflow-hidden">
          <div className="p-4 border-b border-gray-800">
            ...
            {!hasBreakdown && (
              <p className="text-gray-600 text-xs mt-1">Breakdown data not yet available</p>
            )}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800 text-gray-500 text-xs uppercase tracking-wider">
                  ...
                  <th className="px-3 py-3 text-center"><span style={{ color: '#10a37f' }}>ChatGPT</span><span className="block font-normal text-gray-600">/30</span></th>
                  <th className="px-3 py-3 text-center"><span style={{ color: '#c9a96e' }}>Claude</span><span className="block font-normal text-gray-600">/30</span></th>
                  <th className="px-3 py-3 text-center"><span style={{ color: '#20b2aa' }}>Perplexity</span><span className="block font-normal text-gray-600">/30</span></th>
                  <th className="px-3 py-3 text-center"><span style={{ color: '#4285f4' }}>Gemini</span><span className="block font-normal text-gray-600">/30</span></th>
                  <th className="px-3 py-3 text-center">Total<span className="block font-normal text-gray-600">/120</span></th>
                  <th className="px-3 py-3 text-center">Rate %</th>
                  <th className="px-3 py-3 text-center">RAW Score<span className="block font-normal text-gray-600">/100</span></th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={pub.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                      ...
                      {['chatgpt','claude','perplexity','gemini'].map(platform => {
                        const color = { chatgpt:'#10a37f', claude:'#c9a96e', perplexity:'#20b2aa', gemini:'#4285f4' }[platform];
                        return (
                          <td key={platform} className="px-3 py-3 text-center">
                            {b ? (
                              <div className="flex flex-col items-center gap-1">
                                <span className="font-mono text-sm" style={{ color }}>{b[platform]}</span>
                                <div className="w-12 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                                  <div className="h-full rounded-full" style={{ width: `${(b[platform]/30)*100}%`, background: color }} />
                                </div>
                              </div>
        ...
        {hasBreakdown && (
          <div className="bg-gray-900/50 rounded-lg border border-gray-800 p-5">
            ...
            <div className="flex gap-6 mb-5 text-xs">
              {[{label:'ChatGPT',color:'#10a37f'},{label:'Claude',color:'#c9a96e'},{label:'Perplexity',color:'#20b2aa'},{label:'Gemini',color:'#4285f4'}].map(({label,color}) => (
                ...
                return (
                  <div key={pub.id}>
                    ...
                    <div className="ml-8 flex h-4 rounded overflow-hidden bg-gray-800">
                      {[{val:b.chatgpt,color:'#10a37f'},{val:b.claude,color:'#c9a96e'},{val:b.perplexity,color:'#20b2aa'},{val:b.gemini,color:'#4285f4'}].map(({val,color},i) =>
                        val > 0 ? <div key={i} style={{ width: `${(val/120)*100}%`, background: color }} title={`${val}/30`} /> : null
                      )}
                    </div>
                  </div>
                );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-4 bg-red-900/30 border border-red-800 rounded-lg p-3 text-red-400 text-sm">
            {error}
            <button onClick={() => setError(null)} className="ml-4 text-red-600 hover:text-red-400">x</button>
          </div>
        )}
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          ...
          <button onClick={() => setSignalPage('matrix')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${signalPage === 'matrix' ? 'text-white' : 'bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-gray-300'}`}
            style={signalPage === 'matrix' ? { background: '#534AB7' } : {}}>
            ◈ Matrix
          </button>
        ...
        {/* Page routing */}
        {signalPage === 'matrix' && <MatrixPanel />}
        {signalPage === 'queryResults' && <QueryResultsPanel />}
        {signalPage === 'aiCitations' && <AICitationsPanel pubs={publications} breakdown={aiBreakdown} />}

        {signalPage && signalPage !== 'aiCitations' && signalPage !== 'queryResults' && signalPage !== 'matrix' && (() => {
...
function updateRow(data) {
  ...
  sheet.getRange(row, 4).setValue(data.scores[0]);
  sheet.getRange(row, 5).setValue(data.scores[1]);
  sheet.getRange(row, 6).setValue(data.scores[2]);
  sheet.getRange(row, 7).setValue(data.scores[3]);
  sheet.getRange(row, 8).setValue(data.scores[4]);
  sheet.getRange(row, 9).setValue(data.scores[5]);
